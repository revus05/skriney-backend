...
public class BalanceService {
    ...
    public List<DailyBalanceDTO> getUserBalanceTimeline(String userUuid, BalancePeriod period, String bankAccountUuid) {
        BigDecimal currentBalance = getCurrentBalance(userUuid, bankAccountUuid);
        Instant fromDateTime = calculateFromDateTime(period);

        List<Transaction> transactions = fetchTransactionsDescending(userUuid, bankAccountUuid, fromDateTime);
        ...
        for (Transaction tx : transactions) {
            ...
            daySummaryMap.computeIfAbsent(date, k -> new DaySummary())
    ...
    private BigDecimal getCurrentBalance(String userUuid, String bankAccountUuid) {
        if (bankAccountUuid == null || bankAccountUuid.isBlank()) {
            return bankAccountRepo.sumBalanceByUserUuid(userUuid)
                    .orElse(BigDecimal.ZERO);
        } else {
            return bankAccountRepo.findByUuidAndUser_Uuid(bankAccountUuid, userUuid)
                    .map(BankAccount::getBalance)
                    .orElse(BigDecimal.ZERO);
        }
    }

    private List<Transaction> fetchTransactionsDescending(String userUuid, String bankAccountUuid, Instant fromDateTime) {
        if (bankAccountUuid == null || bankAccountUuid.isBlank()) {
            return transactionRepo
                    .findByUser_UuidAndCreatedAtGreaterThanEqualOrderByCreatedAtDesc(userUuid, fromDateTime);
        } else {
            return transactionRepo
